 ...
 *
 */
public class MaterialTabBar__old extends Nav implements HasType<TabBarType>, HasChangeHandlers {

	protected Span indicator = new Span(CssName.MDC_TAB_BAR__INDICATOR);

	protected final TypeMixin<MaterialTabBar__old, TabBarType> typeMixin = new TypeMixin<>(this);
	protected int activeTabIndex = -1;

	public MaterialTabBar__old() {
	...
	protected void loadFirstSelected() {
		...
		if (activeTabIndex > -1 && activeTabIndex < getWidgetCount() - 1) {
			...
			if (widget instanceof MaterialTab__old) {
				((MaterialTab__old) widget).setActive(true);
			}
		}
		...
		for (int i = 0; i < getWidgetCount(); i++) {
			...
			if (child instanceof MaterialTab__old) {
				final MaterialTab__old tab = (MaterialTab__old) child;
				if (tab.isActive()) {
					setActiveTabIndex(i);
				}
			}
	...
	protected void updateTabs(final int oldIndex, final int newIndex) {
		...
		if (oldIndex > -1) {
			...
			if (oldChild instanceof MaterialTab__old) {
				final MaterialTab__old tab = (MaterialTab__old) oldChild;
		...
		if (newIndex > -1) {
			...
			if (newChild instanceof MaterialTab__old) {
				final MaterialTab__old tab = (MaterialTab__old) newChild;
