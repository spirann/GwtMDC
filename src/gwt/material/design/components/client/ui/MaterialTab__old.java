 ...
 *
 */
public class MaterialTab__old extends Anchor implements HasHref, HasText, HasIcon, HasSelectionHandlers<MaterialTab__old> {
	...
	protected final HrefMixin<MaterialTab__old> hrefMixin = new HrefMixin<>(this);
	protected final AttributeMixin<MaterialTab__old> ariaControlsMixin = new AttributeMixin<>(this, "aria-controls");

	protected final ApplyStyleMixin<MaterialTab__old> activeMixin = new ApplyStyleMixin<>(this, CssName.MDC_TAB__ACTIVE);

	private HandlerRegistration handler;

	public MaterialTab__old() {
	...
	protected void onInitialize() {
		...
		if (!(getParent() instanceof MaterialTabBar__old)) {
			super.onInitialize();
		}
	...
	@Override
	public HandlerRegistration addSelectionHandler(SelectionHandler<MaterialTab__old> handler) {
	...
	protected void setActive(boolean active, boolean notifyParent) {
		...
		if (notifyParent && isAttached()) {
			...
			if (widget instanceof MaterialTabBar__old) {
				final MaterialTabBar__old materialTabBar = (MaterialTabBar__old) widget;
