 ...
 *
 */
public class DaysOLF extends Div
		implements HasType<DatePickerType>, HasSelection<MdcDate[]>, HasSelectionHandlers<MdcDate[]> {

	protected final TypeMixin<DaysOLF, DatePickerType> typeMixin = new TypeMixin<>(this, DatePickerType.RANGE);
	protected final HasSelectionMixin<DaysOLF, MdcDate[]> selectionMixin = new HasSelectionMixin<>(this);
	...
	protected void drawDates(final Collection<MdcDate> dates) {
		...
		dates.forEach(date -> {
			...
			item.addClickHandler(event -> {
				...
				final boolean isSelected = selectedDates.contains(item.getDataObject());
	...
	protected void drawRangeSelect(MdcDate... selectedItems) {
		...
		items.values().stream().forEach(item -> {
			...
			final MdcDate date = item.getDataObject();
			final int timestamp = date.getTimestamp();

			if (timestamp >= start.getTimestamp() && timestamp <= end.getTimestamp()) {
				item.addStyleName(CssName.MDC_DATEPICKER__ACTIVE);
				if (item.getDataObject().equals(start))
					item.addStyleName(CssName.MDC_DATEPICKER__ACTIVE_FIRST);
				if (item.getDataObject().equals(end))
					item.addStyleName(CssName.MDC_DATEPICKER__ACTIVE_LAST);
			}
		});
	}
